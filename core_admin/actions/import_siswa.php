...
if (isset($_POST) && isset($_FILES['file_excel'])) {
    ...
    if ($handle !== FALSE) {
        ...
        foreach ($headers as $index => $col_name) {
            ...
            if (strpos($col_name, 'induk') !== false)
                $map['no_induk'] = $index;
            elseif (strpos($col_name, 'nama peserta') !== false || strpos($col_name, 'nama siswa') !== false || $col_name == 'nama')
                $map['nama'] = $index;
            elseif ($col_name == 'l/p' || strpos($col_name, 'kelamin') !== false)
                $map['jk'] = $index;
            elseif (strpos($col_name, 'tempat') !== false)
                $map['kota'] = $index;
            elseif (strpos($col_name, 'tanggal lahir') !== false || strpos($col_name, 'tgl lahir') !== false)
                $map['tgl_lahir'] = $index;
            elseif (strpos($col_name, 'alamat') !== false)
                $map['alamat'] = $index;
            elseif (strpos($col_name, 'ayah') !== false && strpos($col_name, 'pekerjaan') === false)
                $map['nama_ayah'] = $index;
            elseif (strpos($col_name, 'pekerjaan ayah') !== false)
                $map['pekerjaan_ayah'] = $index;
            elseif (strpos($col_name, 'ibu') !== false && strpos($col_name, 'pekerjaan') === false)
                $map['nama_ibu'] = $index;
            elseif (strpos($col_name, 'pekerjaan ibu') !== false)
                $map['pekerjaan_ibu'] = $index;
            elseif (strpos($col_name, 'hp') !== false && strpos($col_name, 'ortu') === false)
                $map['no_hp'] = $index; // Fix regex HP
            elseif (strpos($col_name, 'nik') !== false)
                $map['nik_ortu'] = $index; // Kolom NIK BARU
            elseif (strpos($col_name, 'kelas') !== false)
                $map['kelas'] = $index;
        }
